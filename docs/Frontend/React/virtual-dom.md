...
<mark>Virtual DOM 是一种编程概念。</mark>

在这个概念里， UI 以一种理想化的，或者说 _虚拟的_ 表现形式被保存于内存中。
...
虚拟 DOM 和 JS 对象之间的关系：<mark>前者是一种思想，后者是一种思想的具体实现。</mark>
...
- DOM 对象最终要被浏览器渲染出来之前，浏览器会有很多 [工作](/Frontend/Browser/render) 要做。
- DOM 对象上面的属性也非常非常多
...
虚拟 DOM 只是对真实 UI 的一个描述，回头根据不同的宿主环境，可以执行不同的渲染代码：
...
1. 相较于 DOM 的体积和速度优势
   ...
   - JS 层面的计算的速度，要比 DOM 层面的计算快得多：DOM 对象最终要被浏览器显示出来之前，浏览器会有很多工作要做，这涉及到了浏览器的渲染管线。此外，DOM 上面的属性也是非常多的。
