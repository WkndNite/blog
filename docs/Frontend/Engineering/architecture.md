...
| -------- | -------------------------------------- | ------------------------------------ |
| 页面结构 | 单一 HTML 页面，通过 JS 动态渲染内容   | 每个页面是一个独立 HTML 文件         |
| 用户体验 | 页面切换无刷新，体验流畅               | 每次跳转刷新页面，体验割裂           |
| SEO 优化 | 较复杂，需要 SSR 或预渲染支持          | 原生支持，天然友好                   |
| 加载速度 | 首屏加载慢，需优化打包体积             | 首屏快，按需加载                     |
| 开发模式 | 前后端分离，使用前端框架管理路由与状态 | 前后端耦合高，后端生成页面           |
| 适用场景 | 交互复杂的后台系统、SPA 工具类产品等   | 内容型网站，如新闻、博客、电商展示页 |
...
和前文提到的 SPA 路由思路相似，Garfish 里的 Router 实现思路也是劫持路由，对关键的 API 及事件进行劫持与监听，在路由发生变化的时候展示对应的子应用即可。而加载哪一个字应用，会根据我们在前面基座文件里配置的 `activeWhen` 字段来判断。
...
- 可支持动态引入 Vmok 模块，项目可完全运行时接入而无须构建插，对搭建场景友好。
