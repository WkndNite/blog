...
[Scheduler 的核心源码](https://github.com/facebook/react/blob/main/packages/scheduler/src/forks/Scheduler.js)
...
// 有两个队列分别存储普通任务和延时任务
// 采用了小顶堆算法，保证每次从队列里取出来的都是优先级最高（时间即将过期）
...
function unstable_scheduleCallback(priorityLevel, callback, options) {
  ...
  if (startTime > currentTime) {
    ...
    // 说明不是延时任务
    // 设置了 sortIndex 后，可以在任务队列里面进行一个排序
...
![alt](https://blog-1328542955.cos.ap-shanghai.myqcloud.com/2022-12-29-065931.png)
  ...
  if (scheduledHostCallback !== null) {
    ...
    try {
      ...
      if (hasMoreWork) {
        ...
        // 那么就使用 messageChannel 进行一个 message 事件
        // 的调度，就将任务放入到任务队列里面
        schedulePerformWorkUntilDeadline();
      } else {
        // 说明任务做完了
        ...
        // scheduledHostCallback 之前为 flushWork，设置为 null
      }
    }
  } else {
    isMessageLoopRunning = false;
...
function workLoop(hasTimeRemaining, initialTime) {
  ...
  if (currentTask !== null) {
    // 如果不为空，代表还有更多的任务
    // 那么回头外部的 hasMoreWork 拿到的就也是 true
    return true;
  } else {
    ...
    // 没有进入上面的 if，说明 timerQueue 里面的任务也完了，返回 false
    // 回头外部的 hasMoreWork 拿到的就也是 false
    return false;
  }
}
```
